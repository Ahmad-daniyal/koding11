...
- **Modular** — setiap file punya satu tanggung jawab, komponen terpisah per folder
- **Data-driven** — semua konten halaman berasal dari file JSON, tidak ada hardcode di JS
- **Router-based** — hash routing (`/#/path`) untuk navigasi antar halaman, komponen di-render per route
- **CSS terbatas** — tidak ada file CSS yang melebihi ~150 baris; gunakan partials jika perlu
- **Layout utama tidak di-render ulang** — header dan footer di-mount sekali, hanya konten yang berubah
- **Komponen shared direuse** — jika komponen dibutuhkan di 2+ section/halaman yang berbeda, taruh di `src/components/` (shared) dan import dari sana; jangan duplikasi kode komponen

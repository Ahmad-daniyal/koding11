...
export function ProfileSection(profilePage, profile) {
  const highlights = profilePage.highlights
    .map(
      (h) => `
      <div class="flex items-start gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 transition hover:-translate-y-1 hover:shadow-md">
        ...
        <div>
          <h3 class="font-semibold text-slate-900">${h.label}</h3>
          <p class="mt-1 text-sm leading-6 text-slate-500">${h.detail}</p>
        </div>
      </div>`
    )
    .join('')
  ...
  section.innerHTML = `
    <div class="grid gap-8 lg:grid-cols-[1fr_1.1fr]">
      <div>
        <p class="text-sm font-semibold uppercase tracking-[0.3em] text-blue-600">${profilePage.title}</p>
        <h2 class="mt-3 text-3xl font-semibold text-slate-900">${profilePage.heading}</h2>
        <p class="mt-4 text-base leading-7 text-slate-600">${profilePage.bio}</p>

        <div class="mt-6 flex flex-col gap-3 text-sm text-slate-500">
          <div class="flex items-center gap-2">
            <span class="text-blue-500">${highlightIcons.mapPin}</span>
            <span>${profilePage.location}</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-blue-500">${highlightIcons.briefcase}</span>
